    ...
    with clientes_t:
        ...
        if len(user_artifacts) <= 6:
            try:
                ...
                print(f" Artefactos :{clientes[client_socket].artefactos}")
...
def handle_client(client_socket):
    ...
    while True:
        ...
        if protocol == 'order':
            ...
            print(f'Partes de los comandos: {commands_parts}')
                ...
                if len(commands_parts) >= 3:
                    ...
                    if A and B:
                        ...
                        with clientes_t:
                            ...
                            print(trade_system)
                            offer_msg = f'[SERVER] Trade Offer from {current_user.nombre}: {miartefactoid} {artifacts[miartefactoid]} por {suartefactoid} {artifacts[suartefactoid]}\n(:accept, :reject) or ignore.'
                            current_user.client_socket.send(f'[SERVER] Trade Offer from {current_user.nombre}: {miartefactoid} {artifacts[miartefactoid]} por {suartefactoid} {artifacts[suartefactoid]}'.encode('utf-8'))
                            dest_socket.send(offer_msg.encode('utf-8'))

                    else:
                        if not A:
                            current_user.client_socket.send(f'[SERVER] {miartefactoid} {artifacts[miartefactoid]} no se encuentra en tu inventario.'.encode('utf-8'))
                        if not B:
                            current_user.client_socket.send(f'[SERVER] {suartefactoid} {artifacts[suartefactoid]} no se encuentra en su inventario.'.encode('utf-8'))
           ...
           # Rechazar intercambio 
                ...
                try:
                    with clientes_t:
                        ...
                        # Mandar mensaje a los usuarios
                        sender.send(f'[SERVER] {clientes[receiver].nombre} ha rechazado la oferta.')
                        receiver.send(f'[SERVER] {clientes[sender].nombre} ha rechazado la oferta.')
